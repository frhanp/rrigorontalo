    ...
    <div class="space-y-6">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-slate-900">
                {{-- Tambahkan x-data dengan nilai dari post yang ada --}}
                <form method="POST" action="{{ route('dashboard.posts.update', $post) }}" enctype="multipart/form-data" x-data="{ status: '{{ old('status', $post->status) }}' }">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex items-center space-x-4 mt-1">
                            <label for="draft" class="flex items-center">
                                <input @change="status = 'draft'" id="draft" name="status" type="radio" value="draft" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-slate-300" {{ old('status', $post->status) == 'draft' ? 'checked' : '' }}>
                                <span class="ml-2 text-sm text-slate-600">Draft</span>
                            </label>
                            <label for="published" class="flex items-center">
                                <input @change="status = 'published'" id="published" name="status" type="radio" value="published" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-slate-300" {{ old('status', $post->status) == 'published' ? 'checked' : '' }}>
                    ...
                    <div x-show="status === 'published'" x-transition class="mt-4">
                        ...
                        {{-- Mengisi nilai awal dengan format yang benar untuk input datetime-local --}}
                        <x-text-input id="published_at" class="block mt-1 w-full" type="datetime-local" name="published_at" :value="old('published_at', $post->published_at?->format('Y-m-d\TH:i'))" />
