                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('dashboard.posts.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="content" id="content" rows="10" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>{{ old('content') }}</textarea>
                         ...
                         <div class="mt-4">
                            ...
                            <div class="flex items-center space-x-4 mt-1">
                                <label for="draft" class="flex items-center">
                                    <input id="draft" name="status" type="radio" value="draft" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300" {{ old('status', 'draft') == 'draft' ? 'checked' : '' }}>
                                    <span class="ml-2 text-sm text-gray-600">Draft</span>
                                </label>
                                <label for="published" class="flex items-center">
                                    <input id="published" name="status" type="radio" value="published" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300" {{ old('status') == 'published' ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center justify-end mt-6">
                             <a href="{{ route('dashboard.posts.index') }}" class="text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                {{ __('Batal') }}
                            </a>
