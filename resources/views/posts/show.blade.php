    ...
    <div class="max-w-3xl mx-auto">
        <article class="bg-white p-6 sm:p-10 rounded-xl shadow-xl border border-slate-200">
            ...
            <a href="{{ route('categories.show', $post->category) }}"
                class="inline-block bg-blue-100 text-blue-700 text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full mb-4 hover:bg-blue-200 transition-colors">
                {{ $post->category->name }}
            </a>
            {{-- Judul dan Meta Info --}}
            ...
            <div
                class="prose prose-lg max-w-none text-slate-800 leading-relaxed prose-a:text-blue-600 hover:prose-a:text-blue-800 mb-8">
                {!! $post->content !!}
            </div>
        ...
        <section id="comments" class="bg-white p-6 sm:p-10 rounded-xl shadow-xl border border-slate-200 mt-10">
                ...
                <form action="{{ route('comments.store') }}" method="POST" class="mb-8">
                    ...
                    <div>
                        <textarea name="content" rows="4"
                            class="w-full bg-slate-100 border-slate-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm"
                            placeholder="Tulis komentar Anda sebagai {{ Auth::user()->name }}..." required>{{ old('content') }}</textarea>
                    </div>
                    <div class="mt-4">
            ...
            @else
                <p class="mb-8 text-center text-slate-500"><a href="{{ route('login') }}"
                        class="text-blue-600 hover:underline font-semibold">Login</a> untuk berkomentar.</p>
            @endauth
                ...
                @forelse($post->comments->sortByDesc('created_at') as $comment)
                    <div class="flex space-x-4 border-t border-slate-200 pt-6">
                        <div class="flex-shrink-0">
                            <div
                                class="w-11 h-11 rounded-full bg-slate-200 flex items-center justify-center font-bold text-slate-600 text-lg">
                                {{ strtoupper(substr($comment->user->name, 0, 1)) }}
                            </div>
                        </div>
                        <div>
                            <p class="font-semibold text-slate-900">{{ $comment->user->name }} <span
